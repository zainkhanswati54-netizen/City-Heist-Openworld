...
// Allow the browser one paint before continuing heavy work
...
async function boot() {
  ...
  const beachCrowd     = CrowdSpawner.spawnBeachCrowd(engine.scene, DISTRICT_LAYOUT.beachBounds, 22);
  ...
  // ── Stage 8: Audio setup (not started yet — needs user gesture) ───────
  loading.setProgress(92, 'Preparing audio…');
  await frame();
  ...
  await new Promise(resolve => {
    ...
    // Also start on canvas click/touch after loading dismisses
  ...
  function animate() {
    ...
    trafficCars.forEach(tc => {
      tc.updateTraffic(dt, trafficMult);
    ...
    pedestrians.forEach(ped => { if (typeof ped.update === 'function') ped.update(dt, period); });
...
// ── Helper — apply schedule to all pedestrian types ──────────────────────
